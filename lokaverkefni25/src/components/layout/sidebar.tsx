...
const Sidebar = () => {
  ...
  const handlePlaceOrder = async () => {
    console.log(
      "currentOrder state at beginning of handlePlaceOrder: ",
      currentOrder
    );
  ...
  // Clean up the state and webapp after order, reset and redirect.
  useEffect(() => {
  ...
  }, [currentStage, setCurrentOrder, setCurrentStage,router]);

  return (
    <div className="flex flex-col gap-2 border border-gray-300 rounded-lg bg-white/40 backdrop-blur-2xl md:shadow-md shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] p-4">
      ...
      {currentStage === OrderStage.CONFIRM_ORDER && (
        <div className="flex flex-col gap-2 justify-center items-center ">
          ...
          <button
            ...
            className="w-full bg-button-card hover:bg-button-primary/50 text-white py-2 px-4 rounded disabled:bg-gray-400"
            disabled={
              !currentOrder?.email ||
              !currentOrder ||
              !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(currentOrder.email)
